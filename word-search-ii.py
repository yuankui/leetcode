...
def binarySearch(items, key, target, start, end, delta=0):
    ...
    if targetValue < middleValue:
        return binarySearch(items, key, target, start, middle, delta)
    elif targetValue > middleValue:
        return binarySearch(items, key, target, middle, end, delta)
    else:
        return True, middle
    ...
    def findWordsInner(self, board, words):
        ...
        for y, line in enumerate(board):
            for x, char in enumerate(line):
                self.used[(x, y)] = True
                path.append(char)
                self.search(board, (x,y), 0, words, 0, len(words), path)
                path.pop()
                self.used[(x, y)] = False
                if len(words) == 0:
                    return True
    ...
    def search(self, board, pos, index, words, wordStart, wordStop, path):
        ...
        for direction in Solution.directions:
            ...
            if valid:
                ...
                findNum += self.search(board, newPos, index + 1, words, filteredStart, filteredStop - findNum, path)
                path.pop()
